...
const ServersOrdersTable = ({productId, userId, exFetching, heading, userFilter, productFilter}) => {
    ...
    const getOrders = async()=>{
        try{    
            setIsFetching(true)
            const res = await fetch(`/api/order-code?page=${page}&pageSize=${pageSize}&productId=${filterProduct}&userId=${filterUser}&isValid=${isValid}&search=${search}`,{
                headers: {
                    Authorization: `Bearer ${JSON.parse(user).token}`,
                },
            })
    ...
    const updateOrderValidation = async(orderId, value, index)=>{
        ...
        try{
            ...
            const res = await fetch(`/api/order-code/valid/${orderId}`,{
                method:"PUT",
                headers: {
    ...
    useEffect(() => {
        setPage(1);
    }, [filterProduct, filterUser, exFetching, isValid, searchValue]);
                ...
                <div className='table1-filter'>
                    ...
                    <div className="right-side">
                        ...
                        <form className="select-filter" onSubmit={(e)=>{e.preventDefault()}}>
                            ...
                            {productFilter ? <FilterByProduct type='code' placeholder={'Filter by server'} setFilterUser={setFilterProduct}/> : null}
